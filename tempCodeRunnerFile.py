...
def init_db():
    ...
    # Check if learning_resources table is empty
...
def mental_mood():
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('mental_home'))
        except Exception as e:
            print(f"Mood assessment error: {e}")
            return f"Error: {e}", 500
    
    return render_template('mental/mood.html')
...
def todo():
    try:
        ...
        week_start = (pd.Timestamp.now() - pd.Timedelta(days=pd.Timestamp.now().weekday())).strftime('%Y-%m-%d')
        week_end = (pd.Timestamp.now() + pd.Timedelta(days=6-pd.Timestamp.now().weekday())).strftime('%Y-%m-%d')
...
def mentor_connect():
    ...
    if user.get('mentor_access', False):
        # If user has mentor access, show chat or connect page
        if is_admin:
            # Show admin panel within mentor page
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute('SELECT id, username, email, is_premium, mentor_access, created_at FROM users')
            users = c.fetchall()
            ...
            return render_template('mentor/connect.html', 
                                 users=users, 
                                 requests=requests, 
                                 is_admin=True)
        else:
...
def mentor_request():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('INSERT INTO mentor_requests (user_id, utr) VALUES (?, ?)', 
              (g.current_user['id'], utr))
    conn.commit()
    conn.close()
    
    flash('Payment request submitted! We will verify and activate your access within 24 hours.', 'success')
    return redirect('/mentor-connect')
...
def admin_panel():
    if request.method == 'POST':
        email = request.form.get('email')
        password = request.form.get('password')
        if email == 'yashshelke98320@gmail.com' and password == 'Yash9353':  # Change this password
...
def approve_mentor_request(request_id):
    ...
    c.execute('SELECT user_id FROM mentor_requests WHERE id = ?', (request_id,))
    result = c.fetchone()
    if result:
        user_id = result[0]
        # Update user access
        c.execute('UPDATE users SET mentor_access = 1 WHERE id = ?', (user_id,))
        # Update request status
        c.execute('UPDATE mentor_requests SET status = "approved" WHERE id = ?', (request_id,))
        conn.commit()
...
def reject_mentor_request(request_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('UPDATE mentor_requests SET status = "rejected" WHERE id = ?', (request_id,))
    conn.commit()
    conn.close()
...
def toggle_premium(user_id):
    ...
    if result:
        ...
        conn.commit()
        flash(f'Premium access {"granted" if new_status else "revoked"} for user.', 'success')
...
def toggle_mentor(user_id):
    ...
    c.execute('SELECT mentor_access FROM users WHERE id = ?', (user_id,))
    result = c.fetchone()
    
    if result:
        current_status = result[0]
        new_status = 0 if current_status else 1
        c.execute('UPDATE users SET mentor_access = ? WHERE id = ?', (new_status, user_id))
        conn.commit()
        flash(f'Mentor access {"granted" if new_status else "revoked"} for user.', 'success')
